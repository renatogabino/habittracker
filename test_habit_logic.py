...
# Testes de Criação e Gestão de Hábitos (5 testes)
...
# Testes de Conclusão de Hábitos (4 testes)
...
# Testes da Lógica de Sequência (Streak) (7 testes)
# Renomeado e corrigido para refletir a lógica de "mostrar streak do dia anterior"
def test_streak_shows_previous_day_streak_if_not_done_today():
    tracker = HabitTracker()
    tracker.add_habit("Escrever")
    # Completou ontem e anteontem
    tracker.mark_complete("Escrever", YESTERDAY)
    tracker.mark_complete("Escrever", DAY_BEFORE)
    # Não completou hoje. Deve mostrar a streak que terminou ontem (2 dias).
...
def test_streak_is_zero_if_not_done_today_and_no_streak_yesterday():
    ...
    # Completou apenas anteontem, não ontem.
    tracker.mark_complete("Desenhar", DAY_BEFORE)
    # Não completou hoje. Streak terminando ontem era 0.
...
def test_streak_is_one_if_done_only_today():
    ...
    # Gap na sequência (não feito ontem)
    tracker.mark_complete("Pintar", DAY_BEFORE) 
    # Feito hoje, streak é 1.
...
def test_streak_across_month_boundary_when_done_today():
    ...
    first_of_june = date(2025, 6, 1) # Este será o "today" do teste
...
def test_streak_across_month_boundary_not_done_today():
    ...
    # Não feito em first_of_june. Streak terminando em last_of_may era 2.
